  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      <div className={`flex flex-col gap-3 transition-all duration-300 transform ${isOpen ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-10 opacity-0 scale-90 pointer-events-none'}`}>
        {contacts.map((contact, idx) => (
          <a
            ...
            className="flex items-center gap-3 px-4 py-2.5 rounded-full bg-[#25D366] text-white shadow-lg hover:scale-105 transition-transform"
          >
      ...
      <button
        ...
        style={{ 
          background: isOpen ? '#1E2D45' : '#25D366',
          boxShadow: isOpen ? '0 4px 20px rgba(0,0,0,0.3)' : '0 4px 20px rgba(37,211,102,0.4)' 
        }}
      >
        {!isOpen && <span className="absolute inset-0 rounded-full bg-[#25D366] animate-pulse opacity-40" />}
        {isOpen ? (
